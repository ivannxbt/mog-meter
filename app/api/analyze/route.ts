...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await model.generateContent([
      ...
      {
        text: `Analiza este screenshot. Intensity: ${intensity}. Si hay chat, MODO A (nombres en burbujas). Si es foto grupal sin mensajes, MODO B (ranking derecha→izquierda). Devuelve el JSON.`,
      },
    ]);
